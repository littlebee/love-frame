...
class MessagePreviewButton(object):
    ...
    def __init__(
        ...
        icon = self._get_icon((0, 0), (w, h))

        dt_format, font_size = ("%b. %-d", 24) if half_sized else (DEFAULT_DT_FORMAT, 24)
        dt = DateTime(self.surface, (0, 0), int(av_file.name)/1000,
            fmt = dt_format,
            font_size=24
        )
        ...
        self.renderables.append([
            ...
            Rectangle(self.surface, (0, 0, w, h),
                color = None,
                border_width = 3,
                border_color = color
            ),
        ...
        # everything this component renders is static, so do this once now
        self.renderables.render(0)
    ...
    def handle_pyg_event(self, event):
        # translate position for our surface
